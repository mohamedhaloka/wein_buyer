...
class EditHeaderMarketBottomSheet extends StatelessWidget {
  const EditHeaderMarketBottomSheet({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: () async {
        MarketCubit.of(context).header = null;
        return true;
      },
      child: Padding(
              ...
              children: [
                ...
                BlocBuilder<MarketCubit, MarketState>(
                  buildWhen: (previous, current) =>
                      previous != current && current is SelectedHeaderState,
                  builder: (context, state) {
                    return SizedBox(
                      height: AppSizes.getProportionateScreenWidth(100),
                      child: Image.file(
                        File(MarketCubit.of(context).header!.path),
                        fit: BoxFit.fill,
                      ),
                    );
                  },
                  ...
                  builder: (context, state) {
                    return state is EditMarketDetailsLoading
                        ...
                        : CustomButton(
                            ...
                            radius: 10,
                            onPress: () {
                              MarketCubit.of(context).editHeaderMarket(context);
                            },
                          );
                  },
                ),
